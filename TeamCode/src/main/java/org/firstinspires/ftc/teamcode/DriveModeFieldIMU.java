...
public class DriveModeFieldIMU extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            fL.setPower(getGamepadMoveMagnitude(gamepad1) * Math.sin(getHeading() + (Math.PI/4)) + getGamepadTurnMagnitude(gamepad1));
            bL.setPower(getGamepadMoveMagnitude(gamepad1) * Math.sin(getHeading() - (Math.PI/4)) + getGamepadTurnMagnitude(gamepad1));
            fR.setPower(getGamepadMoveMagnitude(gamepad1) * Math.sin(getHeading() + (Math.PI/4)) - getGamepadTurnMagnitude(gamepad1));
            bR.setPower(getGamepadMoveMagnitude(gamepad1) * Math.sin(getHeading() - (Math.PI/4)) - getGamepadTurnMagnitude(gamepad1));
