...
public class VuMarkIdentification extends LinearOpMode {
  ...
  public class VuMarkFinder {
    ...
    public VuMarkFinder(
       ...
       */
      if (includeViewer) {
        int cameraMonitorViewId = hMap.appContext
          .getResources()
          .getIdentifier(
            "cameraMonitorViewId",
            "id",
    ...
    public boolean findVuMark() {
      ...
      instanceId =
        (
          (VuforiaTrackableDefaultListener) template.getListener()
        ).getVuMarkInstanceId();
      ...
      if (instanceId != null) {
        ...
        if (vuforia.getCameraName().isWebcam()) pose =
          (
            (VuforiaTrackableDefaultListener) template.getListener()
          ).getFtcCameraFromTarget(); else pose =
          ((VuforiaTrackableDefaultListener) template.getListener()).getPose();
