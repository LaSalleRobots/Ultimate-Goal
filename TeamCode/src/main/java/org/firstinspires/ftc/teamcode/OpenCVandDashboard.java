...
public class OpenCVandDashboard extends LinearOpMode {
  ...
  public void runOpMode() throws InterruptedException {
    ...
    VuforiaLocalizer.Parameters parameters =
        new VuforiaLocalizer.Parameters(viewportContainerIds[0]);
    ...
    openCvPassthrough =
        OpenCvCameraFactory.getInstance()
            .createVuforiaPassthrough(vuforia, parameters, viewportContainerIds[1]);

    DistanceFromPaperTracker data =
        new DistanceFromPaperTracker(
            pctThreshold, thresholdThresh, thresholdMaxVal, cannyMinThreshold, cannyMaxThreshold);
    final OpenCvTrackerApiPipeline runner = new OpenCvTrackerApiPipeline();
    runner.addTracker(data);

    openCvPassthrough.openCameraDeviceAsync(
        new OpenCvCamera.AsyncCameraOpenListener() {
          ...
          public void onOpened() {
            ...
            // issues on some devices, though, so if you experience issues you may wish to disable
            // it.
